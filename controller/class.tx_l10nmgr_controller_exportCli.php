...
class tx_l10nmgr_controller_exportCli extends tx_mvc_controller_cli {
	...
	protected $cli_options = array(
		...
		array(
			...
			"Override source language UID defined in localization manager configurations records.\n"
		),
		array(
			'--source',
			'Source language',
			"UID for the source language used during export. Default is 0. In that case UID is taken from localization manager configurations records.\n"
		),
		array(
			'--target',
			'Target languages',
		...
		foreach ($this->getL10nConfigurationIds() as $l10nConfigurationId) {
			foreach ($this->getTargetLanguages() as $targetLanguageId) {
				...
				$exportData['title'] = 'Generated by cli'; /** @todo make configurable */
					$sourceLang = isset ( $this->arguments['--source'] ) ? (int) $this->arguments['--source'][0] : 0;
				$exportData['source_lang'] = $sourceLang;
				...
				$this->cli_echo(sprintf('Creating export data record for configuration "%s", source language "%s" and target language "%s"', $l10nConfigurationId, $sourceLang, $targetLanguageId) . "\n");
