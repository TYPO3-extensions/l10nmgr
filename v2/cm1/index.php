...
$LANG->includeLLFile('EXT:tx_l10nmgr/cm1/locallang.xml');
					...
					if (is_array($data['fields']))	{
						...
						foreach($data['fields'] as $key => $tData)	{
							if (is_array($tData))	{
								...
								if (!$this->MOD_SETTINGS["onlyChangedContent"] || !$noChangeFlag)	{
									reset($tData['previewLanguageValues']);
									//$dataForTranslation=$tData['translationValue'];
									//if ($dataForTranslation=='')
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tx_l10nmgr/cm1/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tx_l10nmgr/cm1/index.php']);
}
